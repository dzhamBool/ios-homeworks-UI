...
class DetailView: UIView {
    struct PostModel { // postModel
        ...
        private lazy var postAuthorLabel: UILabel = { // ЗАГОЛОВОК
            let label = UILabel()
        ...
        private lazy var viewLabel: UILabel = { // ПРОСМОТРЫ
            let label = UILabel()
            label.text  = "Views: "
            label.backgroundColor = .clear
            label.font = UIFont.systemFont(ofSize: 16)
        ...
        private lazy var backButton: UIButton = {  // КНОПКА ВОЗВРАТА
            let button = UIButton()
    ...
    extension DetailView {
        ...
        func setup(with postModel: PostModel) { //
